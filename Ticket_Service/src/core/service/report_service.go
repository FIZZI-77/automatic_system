...
func (s *ReportService) Create(ctx context.Context, in *models.CreateWorkReportInput) (*models.WorkReport, error) {
	...
	if !hasPrivilegedRole(in.ActorRoles) && (ticket.BrigadeID == nil) {
		return nil, models.ErrPermissionDenied
	}
	return s.reports.Create(ctx, in)
}
func (s *ReportService) List(ctx context.Context, ticketID, actor uuid.UUID, roles []string) ([]*models.WorkReport, error) {
	ticket, err := s.tickets.GetTicketByID(ctx, ticketID)
	if err != nil {
		return nil, err
	}
	if !canReadTicket(ticket, &actor, roles) {
		return nil, models.ErrPermissionDenied
	}
