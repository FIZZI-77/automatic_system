...
function Landing({ interactive, onLogin, onRegister, onDemo }: { interactive: boolean; onLogin: () => void; onRegister: () => void; onDemo: (role: Role) => void }) {
  return <main className="landing">
    <nav className="landing-nav"><a className="brand" href="#top"><span>Г</span>Город рядом</a><div className="landing-links"><a href="#how">Как это работает</a><a href="#services">Возможности</a><button className="text-button" disabled={!interactive} onClick={onLogin}>Войти</button><button className="nav-register" disabled={!interactive} onClick={onRegister}>Регистрация</button><button className="primary small" disabled={!interactive} onClick={() => onDemo("dispatcher")}>Открыть демо</button></div></nav>
    <section className="hero" id="top"><div className="hero-copy"><div className="eyebrow">Единая цифровая городская служба</div><h1>Город слышит.<br/><em>Службы действуют.</em></h1><p>Сообщайте о проблемах, следите за исполнением и наблюдайте, как город становится лучше — в одном понятном пространстве.</p><div className="hero-actions"><button className="primary" disabled={!interactive} onClick={() => onDemo("user")}>Сообщить о проблеме <b>→</b></button><button className="ghost" disabled={!interactive} onClick={() => onDemo("dispatcher")}>Посмотреть живую карту</button></div><div className="trust"><span>● Сервисы на связи</span><span>Среднее время реакции <b>18 мин</b></span></div></div>
    ...
    <footer><a className="brand" href="#top"><span>Г</span>Город рядом</a><p>Цифровые городские службы — ближе, понятнее, быстрее.</p><div className="footer-auth"><button className="ghost" disabled={!interactive} onClick={onRegister}>Зарегистрироваться</button><button className="primary small" disabled={!interactive} onClick={onLogin}>Войти в систему</button></div></footer>
  </main>;
}
...
function DashboardV2({ session, role, setDemoRole, onExit }: {session:Session;role:Role;setDemoRole:(r:Role)=>void;onExit:()=>void}) {
  ...
  const pageTitle = section === "overview" ? navByRole[role][0].label : pageTitles[section] || "Городские сервисы";
