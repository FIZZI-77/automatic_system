...
try {
  ...
  foreach ($item in $services) {
    ...
    docker build `
            -t "automatic-system/$name-migrator:dev" `
            -f "$k8sDir/build/Dockerfile.migrator" `
            --build-arg "MIGRATIONS_DIR=scheme" `
            "$repoRoot/$dir"

    if ($LASTEXITCODE -ne 0) {
      throw "Failed to build automatic-system/$name-migrator:dev"
    }
  }
  ...
  docker build `
        -t automatic-system/analytics-clickhouse-init:dev `
        -f "$k8sDir/build/Dockerfile.clickhouse-init" `
        .

  if ($LASTEXITCODE -ne 0) {
    throw "Failed to build automatic-system/analytics-clickhouse-init:dev"
  }
