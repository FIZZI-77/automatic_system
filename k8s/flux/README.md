...
- the `test` branch contains this directory before reconciliation is enabled.
...
After the files are available in the remote `test` branch, the CI deployment job
applies `bootstrap/local-ha-sync.yaml`, waits until Flux has fetched the exact
release commit, and removes the initial suspension.
...
SHA tags, commits the GitOps update with `[skip ci]`, and waits for all 17 Flagger
primary deployments to promote that tag.
