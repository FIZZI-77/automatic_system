...
Если при сохранении прогноза объект переходит из некритичного уровня риска в
`CRITICAL`, репозиторий публикует событие `asset.RISK_BECAME_CRITICAL`.
Опциональный Kafka worker читает это событие и создает связанную заявку в
Ticket Service с `asset_id`. Worker включается только при заданных
`CRITICAL_RISK_TICKET_CATEGORY_ID` и `CRITICAL_RISK_TICKET_REQUESTER_ID`; без
этих значений пересчет риска сохраняет прогноз, но заявку не создает.
