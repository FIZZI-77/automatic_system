...
func New(api analyticsv1.AnalyticsServiceClient) *Client {
...
func (c *Client) Build(ctx context.Context, t models.Type, f models.Filter, roles []string) ([][]string, error) {
	...
	case models.TypeTicketOverview:
		...
		return [][]string{
			{"Metric", "Value"},
			{"Created", fmt.Sprint(x.Created)},
			{"Completed", fmt.Sprint(x.Completed)},
			{"Canceled", fmt.Sprint(x.Canceled)},
			{"Active", fmt.Sprint(x.Active)},
			{"Completion rate", fmt.Sprintf("%.2f%%", x.CompletionRate)},
			{"Average response (sec)", fmt.Sprintf("%.2f", x.AvgResponseSeconds)},
			{"Average resolution (sec)", fmt.Sprintf("%.2f", x.AvgResolutionSeconds)},
		}, nil
	case models.TypeSLASummary:
		...
		return [][]string{
			{"Metric", "Value"},
			{"Response warnings", fmt.Sprint(x.ResponseWarnings)},
			{"Response breaches", fmt.Sprint(x.ResponseBreaches)},
			{"Resolution warnings", fmt.Sprint(x.ResolutionWarnings)},
			{"Resolution breaches", fmt.Sprint(x.ResolutionBreaches)},
			{"Completed", fmt.Sprint(x.Completed)},
			{"Breach rate", fmt.Sprintf("%.2f%%", x.BreachRate)},
		}, nil
	case models.TypeTicketBreakdown:
		x, e := c.api.ListTicketBreakdown(ctx, &analyticsv1.ListTicketBreakdownRequest{
			Filter:    filter,
	...
	case models.TypeDailyTickets:
		...
		rows := [][]string{
			{"Day", "Created", "Completed", "Canceled", "SLA breaches"},
		}
		for _, v := range x.Items {
			rows = append(rows, []string{
				v.Day.AsTime().Format("2006-01-02"),
				fmt.Sprint(v.Created),
				fmt.Sprint(v.Completed),
				fmt.Sprint(v.Canceled),
				fmt.Sprint(v.SlaBreaches),
			})
		}
