...
func (w *Worker) batch(c context.Context) error {
	for range 50 {
		...
		var kind string
		var payload []byte
		e = tx.QueryRow(c, `SELECT id,aggregate_id,event_type,payload FROM outbox_events WHERE status IN('PENDING','FAILED') AND next_attempt_at<=now() ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT 1`).Scan(&id, &aggregate, &kind, &payload)
		...
		e = w.writer.WriteMessages(c, kafka.Message{Key: []byte(aggregate.String()), Value: payload, Headers: []kafka.Header{{Key: "event_id", Value: []byte(id.String())}, {Key: "event_type", Value: []byte(kind)}}})
		if e != nil {
