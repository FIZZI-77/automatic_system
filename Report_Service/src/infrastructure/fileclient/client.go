...
func New(api filev1.FileServiceClient, internal string) *Client {
	return &Client{
		...
		internalEndpoint: strings.TrimRight(internal, "/"),
	}
}
...
func (c *Client) UploadForResource(ctx context.Context, resourceType string, resourceID, owner uuid.UUID, roles []string, a models.Artifact) (uuid.UUID, error) {
	...
	created, e := c.api.CreateUpload(ctx, &filev1.CreateUploadRequest{
	...
	if _, e = c.api.ConfirmUpload(ctx, &filev1.ConfirmUploadRequest{
		FileId:      id.String(),
		ActorUserId: owner.String(),
		ActorRoles:  roles,
	}); e != nil {
		return uuid.Nil, e
	}

	_, e = c.api.LinkFile(ctx, &filev1.LinkFileRequest{
		FileId:       id.String(),
...
func (c *Client) LinkExisting(ctx context.Context, fileID, resourceID, actor uuid.UUID, roles []string) error {
	_, err := c.api.LinkFile(ctx, &filev1.LinkFileRequest{
...
func (c *Client) DownloadImages(ctx context.Context, ids []uuid.UUID, actor uuid.UUID, roles []string) ([]models.EmbeddedImage, error) {
	...
	for _, id := range ids {
		item, err := c.api.GetDownloadURL(ctx, &filev1.GetDownloadURLRequest{
			FileId:      id.String(),
			ActorUserId: actor.String(),
			ActorRoles:  roles,
		})
		...
		result = append(result, models.EmbeddedImage{
			Name:        file.GetName(),
...
func (c *Client) Download(ctx context.Context, id, actor uuid.UUID, roles []string) (string, time.Time, error) {
	x, e := c.api.GetDownloadURL(ctx, &filev1.GetDownloadURLRequest{
		FileId:      id.String(),
		ActorUserId: actor.String(),
		ActorRoles:  roles,
	})
