...
func TestNewValidationAndDefaults(t *testing.T) {
	for _, cfg := range []Config{
		{Topic: "events"},
		{Brokers: []string{" ", ""}, Topic: "events"},
		{Brokers: []string{"kafka:9092"}, Topic: " "},
	} {
	...
	if worker.cfg.BatchSize != defaultBatchSize || worker.cfg.PollInterval != defaultPollInterval ||
		worker.cfg.MaxAttempts != defaultMaxAttempts || worker.cfg.LockTimeout != defaultLockTimeout ||
		worker.cfg.WorkerCount != defaultWorkerCount {
...
func TestNewPreservesPositiveConfiguration(t *testing.T) {
	cfg := Config{Brokers: []string{"kafka:9092"}, Topic: "events", BatchSize: 9,
		PollInterval: 2 * time.Second, MaxAttempts: 3, LockTimeout: time.Minute, WorkerCount: 7}
	worker, err := New(nil, cfg, zap.NewNop())
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = worker.Close() })
	if worker.cfg.BatchSize != 9 || worker.cfg.PollInterval != 2*time.Second ||
		worker.cfg.MaxAttempts != 3 || worker.cfg.LockTimeout != time.Minute || worker.cfg.WorkerCount != 7 {
