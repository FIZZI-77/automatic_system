...
Обычные HTTP-маршруты берутся из `src/core/handlers/handler.go`, а схемы запросов и
типизированных ответов из `models`. После изменения маршрутов, моделей или
обработчиков пересоздайте спецификацию из каталога `API_Gateway`:
...
Проверка `-check` также выполняется в CI. Генератор не придумывает схему ответа,
если обработчик возвращает объект protobuf или динамический `gin.H` без
отдельной HTTP-модели. Такие ответы нужно уточнять при сопровождении API.
Маршрут `/notifications/ws` использует WebSocket и не входит в спецификацию
обычных HTTP-запросов.

Страница `/swagger/` и спецификация `/swagger/openapi.json` доступны только
