...
func NewAssetHandler(c assetv1.AssetServiceClient) *AssetHandler {
	return &AssetHandler{c: c}
}
...
func (h *AssetHandler) Create(c *gin.Context) {
	...
	q := &assetv1.CreateAssetRequest{
		...
		GeometryGeoJson:        v.GeometryGeoJSON,
		Model:                  v.Model,
		SerialNumber:           v.SerialNumber,
		InstallationYear:       v.InstallationYear,
		ServiceLifeYears:       v.ServiceLifeYears,
		Owner:                  v.Owner,
		ServiceOrganization:    v.ServiceOrganization,
		Contractor:             v.Contractor,
		InspectionIntervalDays: v.InspectionIntervalDays,
		ResponseNormMinutes:    v.ResponseNormMinutes,
		RepairNormMinutes:      v.RepairNormMinutes,
...
func (h *AssetHandler) Resolve(c *gin.Context) {
	...
	for offset := int32(0); ; offset += pageSize {
		result, err := h.c.ListAssets(dispatchContext(c), &assetv1.ListAssetsRequest{
			Limit:  pageSize,
			Offset: offset,
		})
		if err != nil {
		...
		for _, asset := range result.GetAssets() {
			idMatches := strings.EqualFold(asset.GetId(), identifier)
			externalIDMatches := strings.EqualFold(asset.GetExternalId(), identifier)
			serialNumberMatches := strings.EqualFold(asset.GetSerialNumber(), identifier)

			if idMatches || externalIDMatches || serialNumberMatches {
				c.JSON(http.StatusOK, &assetv1.AssetResponse{
					Asset: asset,
				})
				return
			}
...
func (h *AssetHandler) Update(c *gin.Context) {
	...
	u, r := principal(c)
	x, e := h.c.UpdateAsset(dispatchContext(c), &assetv1.UpdateAssetRequest{
		AssetId:         v.AssetID,
		ActorUserId:     u,
		ActorRoles:      r,
		Name:            v.Name,
...
func (h *AssetHandler) ChangeStatus(c *gin.Context) {
	...
	x, e := h.c.ChangeAssetStatus(dispatchContext(c), &assetv1.ChangeAssetStatusRequest{
		AssetId:     v.AssetID,
		Status:      st,
		ActorUserId: u,
		ActorRoles:  r,
...
func (h *AssetHandler) Nearby(c *gin.Context) {
	...
	x, e := h.c.FindNearbyAssets(dispatchContext(c), &assetv1.FindNearbyAssetsRequest{
		Latitude:     v.Latitude,
...
func (h *AssetHandler) Incident(c *gin.Context) {
	...
	u, r := principal(c)
	x, e := h.c.RecordIncident(dispatchContext(c), &assetv1.RecordIncidentRequest{
		AssetId:     v.AssetID,
		TicketId:    v.TicketID,
...
func (h *AssetHandler) Repair(c *gin.Context) {
	...
	u, r := principal(c)
	x, e := h.c.CompleteRepair(dispatchContext(c), &assetv1.CompleteRepairRequest{
		AssetId:            v.AssetID,
		IncidentId:         v.IncidentID,
		TicketId:           v.TicketID,
		BrigadeId:          v.BrigadeID,
		Description:        v.Description,
		ReplacedComponents: v.ReplacedComponents,
		DurationMinutes:    v.DurationMinutes,
		CompletedAt:        ts(v.CompletedAt),
		ActorUserId:        u,
		ActorRoles:         r,
	})
	dispatchResponse(c, http.StatusCreated, e, x)
...
func (h *AssetHandler) Inspection(c *gin.Context) {
	...
	u, r := principal(c)
	x, e := h.c.RecordInspection(dispatchContext(c), &assetv1.RecordInspectionRequest{
		AssetId:         v.AssetID,
		InspectorUserId: u,
		Kind:            v.Kind,
		Result:          v.Result,
		DefectFound:     v.DefectFound,
		ConditionScore:  v.ConditionScore,
		Recommendation:  v.Recommendation,
		InspectedAt:     ts(v.InspectedAt),
		ActorRoles:      r,
	})
	dispatchResponse(c, http.StatusCreated, e, x)
}
...
func (h *AssetHandler) CreatePlan(c *gin.Context) {
	...
	u, r := principal(c)
	x, e := h.c.CreateMaintenancePlan(dispatchContext(c), &assetv1.CreateMaintenancePlanRequest{
		AssetId:      v.AssetID,
		Kind:         v.Kind,
		IntervalDays: v.IntervalDays,
		NextDueAt:    timestamppb.New(v.NextDueAt),
		ActorUserId:  u,
		ActorRoles:   r,
	})
	dispatchResponse(c, http.StatusCreated, e, x)
...
func (h *AssetHandler) DuePlans(c *gin.Context) {
	...
	x, e := h.c.ListDueMaintenance(dispatchContext(c), &assetv1.ListDueMaintenanceRequest{
		DepartmentId: v.DepartmentID,
		DueBefore:    timestamppb.New(v.DueBefore),
		Limit:        v.Limit,
		Offset:       v.Offset,
	})
	dispatchResponse(c, http.StatusOK, e, x)
...
func (h *AssetHandler) Recalculate(c *gin.Context) {
	...
	_, r := principal(c)
	x, e := h.c.RecalculateRisks(dispatchContext(c), &assetv1.RecalculateRisksRequest{
		DepartmentId: v.DepartmentID,
		ActorRoles:   r,
	})
	dispatchResponse(c, http.StatusOK, e, x)
