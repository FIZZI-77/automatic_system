...
func (h *LocationHandler) RecordPosition(c *gin.Context) {
	...
	res, err := h.client.RecordPosition(locationRequestContext(c), &locationv1.RecordPositionRequest{
		EventId:        req.EventID,
	...
	locationResponse(c, http.StatusCreated, nil, gin.H{
		"position":  fromProtoPosition(res.GetPosition()),
...
func (h *LocationHandler) GetCurrentLocation(c *gin.Context) {
	...
	locationResponse(c, http.StatusOK, nil, gin.H{
		"location": fromProtoCurrentLocation(res.GetLocation()),
	})
}
...
func (h *LocationHandler) GetCurrentLocations(c *gin.Context) {
	...
	locationResponse(c, http.StatusOK, nil, gin.H{
		"locations":           locations,
		"missing_brigade_ids": res.GetMissingBrigadeIds(),
	})
}
...
func (h *LocationHandler) FindNearbyBrigades(c *gin.Context) {
	...
	for _, value := range res.GetBrigades() {
		brigades = append(brigades, &models.NearbyBrigade{
			BrigadeID:      value.GetBrigadeId(),
			Location:       fromProtoCurrentLocation(value.GetLocation()),
			DistanceMeters: value.GetDistanceMeters(),
		})
	}
...
func (h *LocationHandler) CreateGeoZone(c *gin.Context) {
	...
	locationResponse(c, http.StatusCreated, nil, gin.H{
		"zone": fromProtoGeoZone(res.GetZone()),
	})
}
...
func (h *LocationHandler) UpdateGeoZone(c *gin.Context) {
	...
	res, err := h.client.UpdateGeoZone(locationRequestContext(c), &locationv1.UpdateGeoZoneRequest{
		Id:      req.ID,
		Name:    req.Name,
		GeoJson: req.GeoJSON,
		Active:  req.Active,
...
func (h *LocationHandler) ListGeoZones(c *gin.Context) {
	...
	locationResponse(c, http.StatusOK, nil, gin.H{
		"zones": fromProtoGeoZones(res.GetZones()),
		"total": res.GetTotal(),
	})
}
